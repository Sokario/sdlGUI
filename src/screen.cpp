...
Screen::Screen(const char* name, int width, int height) {
    ...
    if (SDL_WasInit(SDL_SUBSYTEM_MASK)) {
        ...
        assignRenderer(SDL_CreateRenderer(m_window, -1, SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC));
...
void Screen::assignWindow(SDL_Window* window) {
    m_window = window;
}
...
void Screen::assignRenderer(SDL_Renderer* renderer) {
    m_renderer = renderer;
}
...
void Screen::setPitch(int pitch) {
    m_pitch = pitch;
}
...
void Screen::computeEvent(SDL_Event event) {
    if (event.window.windowID == getWindowId()) {
        setQuitButton(event.button.x, event.button.y);
        Widget* widget = NULL;
        int indice = 0, cpt = 0;
        for (int it = 0; it < m_widget.size(); it++) {
        ...
        if ((event.button.button == SDL_BUTTON_LEFT) && (event.button.state == SDL_PRESSED) && getQuitButton()) {
            SDL_Event sdl_event;
